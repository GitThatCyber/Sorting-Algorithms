...
*/
int ArrayList::sequentialSearch(int x)
{
	for (int i=0; i < length; i++)
		if (list[i] == x)
			return i + 1;	// x is in the array
...
* Returns true if x is found in the list, false otherwise.
*/
int ArrayList::sortedSearch(int x)
{
	int i = 0;
...
int ArrayList::elementPeek(int element)
{
	return list[element];
}


int ArrayList::compare(int x)
{
	return list[x];
}